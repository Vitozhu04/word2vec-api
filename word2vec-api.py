...
class MostSimilar(Resource):
    def get(self):
        ...
        parser.add_argument('topn', type=int, required=False, help="Number of results.", location=['args'])
        args = parser.parse_args()
...
class ModelWordSet(Resource):
    def get(self):
        try:
            ...
            print(e)
            return
class ExtractPosTags(Resource):
    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument('sentence', type=str, required=True, help="sentence cannot be blank!", location=['args'])
        args = parser.parse_args()
        #return keywords(args['sentence'])
        tokenizedWords = word_tokenize(args['sentence'])
        pos_tags = nltk.pos_tag(tokenizedWords)
        return pos_tags
...
if __name__ == '__main__':
    ...
    api.add_resource(ExtractPosTags, '/word2vec/extract_postags')
    app.run(host=host, port=port, threaded=10)
